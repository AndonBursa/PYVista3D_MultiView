...
class PyVistaIndustrialViewer(QMainWindow):
    ...
    def initialize_real_plc(self, file_path):
        """🚀 DÜZELTME: Sonsuz bağlantı döngüsünü engellemek için koruma bloğuna alındı"""
        ...
        if self.plc_manager is None:
            if PLC_LIBRARY_LOADED:
                try:
                    ...
                    self.current_plc_type = PLCManagerFactory.get_saved_plc_type(self.json_path)
                    self.seperate_dict()
                    ...
                    print("✅ PLC Manager başarıyla ilk kez oluşturuldu.")
